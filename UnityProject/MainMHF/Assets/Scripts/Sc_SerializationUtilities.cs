...
namespace GalacticWar
{
    namespace SerializationUtilities
    {
        ...
        public struct SerializedNavMeshConvexPolygon
        {
            ...
            public SerializedNavMeshConvexPolygon(NavMeshConvexPolygon v)
            {
            ...
            public static explicit operator SerializedNavMeshConvexPolygon(NavMeshConvexPolygon v)
            {
                return new SerializedNavMeshConvexPolygon(v);
            }

            public static explicit operator NavMeshConvexPolygon(SerializedNavMeshConvexPolygon v)
            {
                var nmcp = new NavMeshConvexPolygon();
        ...
        [Serializable]
        public struct SerializedNavMeshData
        {
            ...
            public SerializedNavMeshData(NavMesh nm)
            {
                navMeshGraph = nm.navMeshGraph;
                polygonsToEdge = nm.polygonsToEdge;
                vertices = new SerializedVector3[nm.vertices.Length];
                for (int i = 0; i < nm.vertices.Length; ++i)
                {
                    vertices[i] = (SerializedVector3)nm.vertices[i];
                }
            }
        }
