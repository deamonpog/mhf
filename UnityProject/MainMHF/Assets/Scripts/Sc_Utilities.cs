...
namespace GalacticWar
{
    public static class Sc_Utilities
    {
        ...
        public static Quaternion GetCourse(Transform in_CurrentTransform, Sc_GeographicCoord in_CurrentGeo, Sc_GeographicCoord in_DestinationGeo)
        {
            // find north at this point
            var scnorth = Sc_SphericalCoord.FromCartesian(in_CurrentTransform.position);
            scnorth.polar -= 1.0f * Mathf.Deg2Rad;
            Vector3 north = scnorth.ToCartesian();

            // lon12: difference between longitudes between two points
            float lon12 = in_DestinationGeo.lon - in_CurrentGeo.lon;

            // alpha1 : course (angle) at this start position
            float alpha1y = Mathf.Cos(in_DestinationGeo.lat) * Mathf.Sin(lon12);
            float alpha1x = Mathf.Cos(in_CurrentGeo.lat) * Mathf.Sin(in_DestinationGeo.lat) - Mathf.Sin(in_CurrentGeo.lat) * Mathf.Cos(in_DestinationGeo.lat) * Mathf.Cos(lon12);
            float alpha1 = Mathf.Atan2(alpha1y, alpha1x);

            Quaternion qNorth = Quaternion.LookRotation(north - in_CurrentTransform.position, in_CurrentTransform.up);
